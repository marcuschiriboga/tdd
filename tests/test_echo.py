...
class TestEcho(unittest.TestCase):
    ...
    def test_lower_long(self):
        """check if arg passes --lower commands to lowercase"""
        args = ["--lower", "Hello World"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "hello world")

    def test_upper_short(self):
        """check if arg passes -u commands to uppercase"""
        args = ["-u", "Hello World"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "HELLO WORLD")

    def test_upper_long(self):
        """check if arg passes --upper commands to uppercase"""
        args = ["--upper", "Hello World"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "HELLO WORLD")

    def test_title_short(self):
        """check if arg passes -t commands to titlecase"""
        args = ["-t", "hello world"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "Hello World")

    def test_title_long(self):
        """check if arg passes --title commands to titlecase"""
        args = ["--title", "hello world"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "Hello World")

    def test_multiple_options(self):
        """check if arg passes multiple commands in order"""
    ...
    def test_help_message(self):
        """ for no options or -h show help commands """
        args = ["-h"]
        output = run_capture(self.module.__file__, args)
